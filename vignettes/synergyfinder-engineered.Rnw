...
\title{User instructions and tutorials of the \textit{synergyfinderengineered} package}
...
%\VignetteIndexEntry{synergyfinderengineered}
\begin{document}
...
@
The PlotDoseResponse function outputs a high-resolution pdf file if the \textit{save.file} parameter is TRUE:
<<fig.show='hide'>>=
...
\begin{figure}[ht!]
  \label{dose}
  \includegraphics[width=1\textwidth]{ispinesib.ibrutinib.dose.response.1.pdf}
  \includegraphics[width=1\textwidth]{canertinib.ibrutinib.dose.response.2.pdf}
...
Once $y_e$ can be determined, the synergy score can be calculated as the difference between the observed effect and the expected effect. For a given dose-response matrix, apply the \textit{CalculateSynergy} function with the desired reference model to calculate the corresponding synergy score at each dose
combination.  Because the input data has been normalized as \%inhibition values, the resulting synergy
scores can be directly interpreted as the proportion of cellular responses attributable to drug interactions.
The ZIP-based synergy scores for the example data can be obtained with the following directive:
<<>>=
  synergy.score <- CalculateSynergy(data = dose.response.mat,
                                    ...
                                    Emin = 0, Emax = 100)
@
The parameters \textit{Emin} and \textit{Emax} specify the minimal and maximal effect of the drug for
purposes of fitting in the four-parameter log-logistic model. In the above example command, the minimal effect is fixed to 0 and the maximal effect is fixed to 100.  The \textit{method} parameter may be set
to 'HSA', 'Loewe' or 'Bliss' to assess the synergy scores with the other reference models.
The \textit{correction} parameter specifies whether or not baseline correction for negative response
values is applied to the raw dose-response data. The 'baseline' for this correction is the average of
the minimum responses of the two single drugs.  The output \textit{synergy.score} contains the following
components:
\begin{itemize}
  \item dose.response.mats: the dose-reponse matrix reshaped from the original input.
  \item drug.pairs: the information of the drug combination such as drug names and drug concentrations.
...
<<fig.show='hide'>>=
  PlotSynergy(synergy.score, type = "2D", save.file = FALSE)
  PlotSynergy(synergy.score, type = "2D", save.file = TRUE)
  PlotSynergy(synergy.score, type = "3D", save.file = FALSE)
  PlotSynergy(synergy.score, type = "3D", save.file = TRUE)
  PlotSynergy(synergy.score, type = "all", save.file = FALSE)
...
\begin{figure}[ht!]
  \label{synergyplot}
  \includegraphics[width=1\textwidth]{ispinesib.ibrutinib.synergy.1.ZIP.all.pdf}
  \includegraphics[width=1\textwidth]{canertinib.ibrutinib.synergy.2.ZIP.all.pdf}
  \caption{\textbf{Drug interaction landscapes based on the ZIP model.} (A) The ibrutinib and ispinesib combination. (B) The ibrutinib and canertinib combination.}
\end{figure}

\section{Summary}
In this vignette, we demonstrated how to use the package \textit{synergyfinderengineered} with an example high-throughput drug combination screening data from [7]. We followed the procedure: illustrating input data, visualising input data, scoring input data and visualising synergy scores. Please go to [2, 6] for more information about the scoring methods.
