...
<<>>=
  synergy.score <- CalculateSynergy(data = dose.response.mat,
                                      ...
                                      Emin = 0, Emax = 100)
@
The parameters \textit{Emin} and \textit{Emax} specify the minimal and maximal effect of the drug for fitting the four-parameter log-logistic model. In the above example command, the minimal effect is fixed to 0 and the maximal effect is fixed to 100. 
\\ \\
